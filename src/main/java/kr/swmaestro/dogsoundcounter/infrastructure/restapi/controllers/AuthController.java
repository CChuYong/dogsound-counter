...
public class AuthController {
    ...
    CompletableFuture<ApiResponse> register(@RequestBody SignUpRequest signUpRequest) {
        if (signUpRequest.getUsername() == null || signUpRequest.getUsername().length() < 4)
            return CompletableFuture.completedFuture(ApiResponse.error("아이디는 4자리 이상으로 설정해주세요"));
        if (signUpRequest.getPassword() == null || signUpRequest.getPassword().length() < 4)
            return CompletableFuture.completedFuture(ApiResponse.error("비밀번호는 4자리 이상으로 설정해주세요"));
    ...
    CompletableFuture<ApiResponse> register(@RequestBody LoginRequest loginRequest) {
        if (loginRequest.getUsername() == null || loginRequest.getUsername().length() < 4)
            return CompletableFuture.completedFuture(ApiResponse.error("입력 정보 오류"));
